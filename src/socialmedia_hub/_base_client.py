...
from socialmedia_hub._logging import rate_limit_logger, requests_logger, retries_logger
...
def _resolve_api_key(explicit: str | None) -> str:
    ...
    raise SMHConfigError(
        "No API key provided. Pass api_key= to SocialMediaHub(...) or set $SMH_API_KEY."
    )
...
def _build_user_agent(explicit: str | None) -> str:
    if explicit:
        return explicit
    return f"socialmedia-hub-py/{__version__}"
...
def _raise_for_status(
    ...
    *,
    ...
    """Translate a non-2xx response into the right SMHHTTPError."""
    ...
    if exc_cls is SMHRateLimitError:
    ...
    if isinstance(body, dict):
        for key in ("message", "error", "detail", "msg"):
            ...
            if isinstance(value, dict):
                sub: str | None = value.get("message") or value.get("msg")
                if sub:
                    return sub
...
class BaseClient:
    """Synchronous base client. ``SocialMediaHub`` subclasses this."""
    ...
    def __init__(
        ...
        *,
        ...
        if http_client is not None and transport is not None:
            raise SMHConfigError("Pass either http_client or transport, not both.")
        ...
        *,
        ...
        while True:
            ...
            try:
                ...
                error: SMHError = from_httpx_request_error(exc)
            except SMHHTTPError as exc:
                error = exc
            ...
            if isinstance(error, SMHRateLimitError):
                rate_limit_logger.warning(
                    "Rate limited on %s %s — sleeping %.2fs (attempt %d/%d)",
                    method, url, sleep, attempt, self._retry_policy.max_retries,
                )
            else:
                retries_logger.info(
...
class AsyncBaseClient:
    """Asynchronous base client. ``AsyncSocialMediaHub`` subclasses this."""
